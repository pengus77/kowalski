...
static int aat1270_probe(struct platform_device *pdev)
{
	...
	if (res < 0) {
		pr_err("[aat1270] gpio_request : gpio_flen=%d, ret=%x \n",pdata->gpio_flen,res);
	}
	res = gpio_direction_output(pdata->gpio_flen, 0);
	if (res < 0) {
		pr_err("[aat1270] gpio_direction_output : flen_gpio_num=%d, ret=%x \n",pdata->gpio_flen,res);
	}

	tegra_gpio_enable(pdata->gpio_enset);
	res = gpio_request(pdata->gpio_enset, "flash_enset");
	if (res < 0) {
		pr_err("[aat1270] gpio_request : gpio_enset=%d, ret=%x \n",pdata->gpio_enset,res);
	}
	res = gpio_direction_output(pdata->gpio_enset, 0);
	if (res < 0) {
		pr_err("[aat1270] gpio_direction_output : enset_gpio_num=%d, ret=%x \n",pdata->gpio_enset,res);
	}


	res = device_create_file(&pdev->dev, &dev_attr_flash);
	if(res){
		pr_err("[aat1270] device create file flash fail!\n");
		goto exit;
	}

	res = device_create_file(&pdev->dev, &dev_attr_flash_brightness);
	if(res){
		pr_err("[aat1270] device create file flash_brightness fail!\n");
		goto exit;
	}

	res = device_create_file(&pdev->dev, &dev_attr_torch);
	if(res){
		pr_err("[aat1270] device create file torch fail!\n");
		goto exit;
	}

	res = device_create_file(&pdev->dev, &dev_attr_enable);
	if(res){
		pr_err("[aat1270] device create file enable fail!\n");
		goto exit;
	}
