...
void cp_power_down(void){
	printk(KERN_DEBUG "=============== cp_power_down ================\n");
	// PMU Reset Pin Low
...
void ifx_power_low(void)
{
	...
	printk(KERN_DEBUG "ifx_power_low - [CP POWER]: pin %d\n",status);  
	printk(KERN_DEBUG "%s: FOTA_proc_excute \n", __func__);
	return;
}
...
void ifx_power_high(void)
{
	...
	status = gpio_get_value(MODEM_GPIO_PWRON);
	printk(KERN_DEBUG "ifx_power_high - [CP POWER]: pin %d\n",status);
	return;	
}
...
void ifx_pmu_reset(void)
{
	...
	printk(KERN_DEBUG "(%d-%02d-%02d %02d:%02d:%02d.%09lu UTC)\n", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
void ifx_fota_reset_long(void)
{
	...
	printk(KERN_DEBUG "%s: CP Reset IN\n", __func__);
	...
	gpio_set_value(MODEM_GPIO_PWRON, 0);
	status = gpio_get_value(MODEM_GPIO_PWRON);
	printk(KERN_DEBUG "%s: MODEM_GPIO_PWRON low- [CP POWER]: pin %d\n", __func__, status);

	mdelay(4000); // 4000mS delay

	gpio_set_value(MODEM_GPIO_PWRON, 1);
	status = gpio_get_value(MODEM_GPIO_PWRON);
	printk(KERN_DEBUG "%s: MODEM_GPIO_PWRON high+ [CP POWER]: pin %d\n", __func__, status);
	...
	printk(KERN_DEBUG "%s: MODEM_GPIO_PWRON_SW low- [CP POWER]: pin %d\n", __func__, status);

	mdelay(1000); // 1000mS delay

	gpio_set_value(MODEM_GPIO_PWRON_SW, 1);
	status = gpio_get_value(MODEM_GPIO_PWRON_SW);
	printk(KERN_DEBUG "%s: MODEM_GPIO_PWRON_SW high+ [CP POWER]: pin %d\n", __func__, status);
	mdelay(200);

	usif_switch_ctrl(0);

	printk(KERN_DEBUG "%s: CP Reset OUT\n", __func__);
}
...
static ssize_t fota_test_proc_write(struct file *filp, const char *buf, size_t len, loff_t *off)
{
	...
	printk(KERN_DEBUG "%s: FOTA_proc_write \n", __func__);
	...
	switch(cmd){
		...
		default :
			printk(KERN_DEBUG "FOTA Driver invalid arg\n");
			break;
	}
	return len;
}
