		...
		unsigned long event, void *data)
{
	...
	if(data)
	{
		printk(KERN_DEBUG "star_reboot_notify : data = %s\n", (unsigned char *)data);
...
{
	int rc = -1;
	printk(KERN_DEBUG "star_setup_reboot\n");
...
static ssize_t star_rs_reset_read(struct device *dev, struct device_attribute *attr, const char *buf)
{
	...
	printk(KERN_DEBUG "star_rs_reset_read : %s\n", (unsigned char*)rsbuf);
	...
	if ('w' == rsbuf[0])
	{
		switch (rsbuf[1])
		{
			case 'm' : // reboot immediately
				printk(KERN_DEBUG "star_rs_reset : reboot immediately detected\n");
				rs = _HIDDEN_RESET;
				rsbuf[2] = 0;
				break;
			case 'a' : // panic
				printk(KERN_DEBUG "star_rs_reset : panic detected\n");
				rs = _HIDDEN_RESET;
				rsbuf[2] = 0;
				break;
			case 'e' : // recovery
				printk(KERN_DEBUG "star_rs_reset : factory reset detected\n");
				rs = _NORMAL_WARMBOOT;
				rsbuf[2] = 0;
				break;
			default : // reboot other case
				printk(KERN_DEBUG "star_rs_reset : warm boot detected\n");
				rs = _NORMAL_WARMBOOT;
				memset(rsbuf+1,0,2);
				break;
		}
	...
	{   
		printk(KERN_DEBUG "star_rs_reset : cold boot detected\n");
		rs = _COLD_BOOT; 
		memset(rsbuf,0,3);
	}  
...
static int star_rs_probe(struct platform_device *pdev)
{
	...
	if (sysfs_create_group(&dev->kobj, &star_rs_reset_attr_group)) {
		...
		printk(KERN_DEBUG "[star_rs] Failed to create sys filesystem\n");
		retval = -ENOSYS;
		goto error;
	}
