...
int lge_nvdata_raw_read(int offset, char* buf, int size)
{
	...
	if(h_file >= 0)
	{
		printk(KERN_DEBUG "read NV size = %d, offset = %d\n",size, offset);	
		sys_lseek( h_file, offset, 0 );

		ret = sys_read( h_file, buf, size);

		printk(KERN_DEBUG "read NV ret = %d\n",ret);	

		if( ret != size )
		{
			printk(KERN_ERR "Can't read  NVDATA.\n");
			return ret;
		}
	...
	{
		printk(KERN_ERR "Can't open  nvdata partition handle = %d.\n",h_file);
		return 0;
	}
	set_fs(oldfs);
	if (size > 1)
		printk(KERN_DEBUG "read NV offset = %d, message = %s(%x)[%d]\n", offset, buf, *buf, *buf);
	else	
		printk(KERN_DEBUG "read NV offset = %d, message = %x[%d]\n", offset, *buf, *buf);
...
int lge_nvdata_raw_write(int offset, char* buf, int size)
{
	...
	if(h_file >= 0)
	{
		printk(KERN_DEBUG "write NV size = %d, offset = %d\n",size, offset);	
		sys_lseek( h_file, offset, 0 );

		ret = sys_write( h_file, buf, size);

		printk(KERN_DEBUG "write NV ret = %d\n",ret);	

		if( ret != size )
		{
			printk(KERN_ERR "Can't write  NVDATA.\n");
			return ret;
		}
	...
	if (size > 1)
		printk(KERN_DEBUG "write NV offset = %d, message = %s(%x)[%d]\n", offset, buf, *buf, *buf);
	else
		printk(KERN_DEBUG "write NV offset = %d, message = %x[%d]\n", offset, *buf, *buf);
...
{
	int pos = (int)offset ;
	printk(KERN_DEBUG "start lge_nvdata_read .\n");
...
{
	int pos = (int)offset;
	printk(KERN_DEBUG "start lge_nvdata_write .\n");
